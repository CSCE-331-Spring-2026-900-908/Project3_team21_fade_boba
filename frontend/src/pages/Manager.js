...
export default function Manager() {
  ...
  const totalRevenue = summary.reduce((sum, day) => sum + parseFloat(day.revenue || 0), 0);
  const totalOrders  = summary.reduce((sum, day) => sum + parseInt(day.order_count || 0, 10), 0);
    ...
    <main style={styles.page} id="main-content">
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerActions}>
          <button style={styles.secondaryButton} onClick={() => loadData(true)}>
            Refresh
          </button>
      ...
      {tab === 'Overview' && (
        <section aria-label="Overview">
          <div style={styles.metricsGrid}>
            ...
            <div style={styles.metricCard}>
              <span style={{ ...styles.metricValue, color: lowStock.length > 0 ? 'var(--red)' : 'var(--green)' }}>
                {lowStock.length}
              </span>
...
const styles = {
  ...
  tabBar:          { display: 'flex', gap: '4px', borderBottom: '1px solid var(--border)', paddingBottom: '0' },
